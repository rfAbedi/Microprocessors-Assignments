...
.DATA
	...
	BARRIER_QUEUE_X DW 31 DUP(?) ; COLLECTION OF BARRIERS THAT APPEAR ON THE SCREEN.
	BARRIER_QUEUE_Y DW 31 DUP(?)
	BARRIER_QUEUE_HEIGHT DW 31 DUP(?)
	
	BARRIER_QUEUE_FRONT DB 0 ; IT'S A CIRCULAR QUEUE.
...
.CODE
	...
	CHECK_COLLISION PROC NEAR
		...
		MOV GAME_OVER, 1
		RET
		CHECK_COLLISION_FOR: ; FOR LOOP WHICH ITERATES BETWEEN BARRIERS AND CHECKS IF THE BALL HAS COLLIDED TO ANY OF THEM.
	...
	MOVE_BARRIERS PROC NEAR
		...
		;MOV CX, [BX][SI] ; READING THE SPEED WHICH BARRIERS SHOULD MOVE.
						 ; THIS IS THE SPEED OF BALL, TO CREAT THE ILLUSION THAT BALL IS STILL MOVING UP, BARRIERS ARE MOVED DOWN WITH THE SAME SPEED(OPPOSITE DIRECTION).
						 ; THERFORE, THE BALL WOULD REMAIN IN ITS POSSITION AND BARRIERS MOVE DOWN AND YOU THINK IT'S THE BALL THAT'S MOVING UP.
	...
	ADD_FRONT PROC NEAR
		MOV AL, BARRIER_QUEUE_FRONT ; CHECKING IF THE QUEUE IS FULL OR NOT.
		INC AL
		MOV AH, 00H
		MOV CL, 31
		DIV CL
	...
	INIT_BARRIER_QUEUE PROC NEAR
		MOV CX, 60
		INIT_BARRIER_FOR: ; A LOOP WHICH ITERATES 13 TIMES.
			CMP CX, 320
			JGE INIT_BARRIER_AFTER ; CX < 0 --> RETURN.
			...
			AND DX, 01H  
			JE RANDOM_NUMBER_EVEN  ; THE ALGORITHM SHOULDN'T PUT ANY BARRIERS INTO THIS SECTION.
			CALL GEN_RANDOM_BARRIER_POS_Y ; THE RANDOM NUMBER WAS ODD, SO, IT IS TIME TO CHOOSE BARRIER_POS_X. 
			MOV AX, RANDOM_NUMBER
			POP CX
			MOV BARRIER_POS_X, CX ; DX = (RANDOM NUMBER) % ( 320(WIDTH OF THE SCREEN) - BARRIER_WIDTH ).
			MOV BARRIER_POS_Y, AX ; BARRIER_POS_Y = NUMBER OF SECTION.
			PUSH CX
	...
	DRAW_SINGLE_BARRIER PROC NEAR
		MOV AH, 0CH ; INTERRUPT FOR WRITING A SINGLE PIXEL ON THE SCREEN.
		MOV DX, BARRIER_POS_Y ; GET A COPY OF BALL_POS_Y.
		...
		DRAW_SINGLE_BARRIER_FOR_1: ; FOR LOOP WHICH ITERATES BETWEEN ROWS.
			...
			MOV CX, BARRIER_POS_X ; GET A COPY OF BALL_POS_X.
